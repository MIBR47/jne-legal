...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $name1 = time() . '-' . $request->file('file_vendor_offer')->getClientOriginalName();
        $name2 = time() . '-' . $request->file('file_mom')->getClientOriginalName();
        $name3 = time() . '-' . $request->file('file_dispotition')->getClientOriginalName();
        $name4 = time() . '-' . $request->file('file_agreement_draft')->getClientOriginalName();
        $name5 = time() . '-' . $request->file('file_customer_entity')->getClientOriginalName();
        $name6 = time() . '-' . $request->file('file_sk_menkumham')->getClientOriginalName();
        $name7 = time() . '-' . $request->file('file_nib')->getClientOriginalName();
        $name8 = time() . '-' . $request->file('file_npwp')->getClientOriginalName();
        $name9 = time() . '-' . $request->file('file_business_permit')->getClientOriginalName();
        $name10 = time() . '-' . $request->file('file_director_id_card')->getClientOriginalName();
        $name11 = time() . '-' . $request->file('file_other')->getClientOriginalName();
